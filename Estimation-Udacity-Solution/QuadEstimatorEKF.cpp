...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  qt.IntegrateBodyRate(V3D(gyro.x, gyro.y, gyro.z), dtIMU);
  float predictedPitch = qt.Pitch();
  float predictedRoll = qt.Roll();
  ekfState(6) = qt.Yaw();	// yaw is already integrated in the IMU update, so we don't need to do it again
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F gyro_interial = attitude.Rotate_BtoI(gyro);
  ...
  predictedState(5) = curState[5] + (accel_inertial.z-CONST_GRAVITY) * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  gPrime(3, QUAD_EKF_NUM_STATES-1) = (RbgPrime(0, 0)*accel_inertial.x + 
                                      RbgPrime(0, 1) * accel_inertial.y + 
                                      RbgPrime(0, 2) * accel_inertial.z)*dt; 

  gPrime(4, QUAD_EKF_NUM_STATES - 1) = (RbgPrime(1, 0) * accel_inertial.x +
                                        RbgPrime(1, 1) * accel_inertial.y +
                                        RbgPrime(1, 2) * accel_inertial.z) * dt; 
  gPrime(5, QUAD_EKF_NUM_STATES - 1) = (RbgPrime(2, 0) * accel_inertial.x +
                                        RbgPrime(2, 1) * accel_inertial.y +
                                        RbgPrime(2, 2) * accel_inertial.z) * dt; 
